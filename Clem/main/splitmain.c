...
/*   Updated: 2020/11/17 18:41:02 by cmoyal           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_nbwords(char *str, char *charset, int *nb_words)
{
	...
	while (str[i])
	{
		if (ft_strstr(charset, str[i - 1]) && !(ft_strstr(charset, str[i])))
			nb_words[0]++;
		i++;
	}
}
...
void	ft_assign_tab(char **splited, char *str, char *charset)
{
	...
	while (*str)
	{
		...
		if (!*str)
			return ;
		while (!ft_strstr(charset, *str) && *str)
		{
			str++;
			len++;
		}
...
int main()
{
	...
	int i;
	i = 0;
	splited = ft_split("!test!test!test", '!');
	while (splited[i])
	{
		printf("%s\n", splited[i]);
		i++;
