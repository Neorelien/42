...
/*   Updated: 2020/11/23 11:26:42 by awery            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "libft.h"

static int			char_in(char a, char charset)
{
	if (a == charset)
		return (1);
	return (0);
...
static void			split(char *str, char charset, char **tableau)
{
	int i;
	int x;
	int y;
...
static void			count_word_lengh(char *str, char charset, char **t)
{
	int i;
	int count;
	int x;
...
char				**ft_split(const char *s1, char charset)
{
	...
	if (nbmot == 0)
	{
		if (!(tableau = (char**)malloc((sizeof(char*) * 1))))
		{
			*tableau = "\0";
			return (tableau);
		}
